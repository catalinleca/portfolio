...
<DetailGrid>
  ...
  <DetailItem label="@hunthub/player-sdk" description="Communication layer between Builder and Explorer. Sends hunt data to the Explorer iframe through typed messages. A server-side transform strips answer keys before anything reaches the Explorer." />
</DetailGrid>
...
<Mermaid chart={`graph TB
  ...
  caption="HedgeHunt monorepo, 4 apps + 3 shared packages"
/>
...
<DecisionCard id="wayfinder" title="Wayfinder: Enforcing the Journey">
<CardField label="Context">
...
<CardBreakout>
  <ProcessFlow title="Traditional Agent Loop">
    <ProcessStep label="Inference 1" detail="Decide to read hunt" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
    <ProcessStep label="Inference 2" detail="Decide to save hunt" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
    <ProcessStep label="Inference 3" detail="Decide to enrich locations" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
...
<CodeShowcase title="Play API" description="Each API response tells the Explorer what it can do next. The Explorer follows server-provided links and never decides on its own. Game rules live on the server, so they can change without touching the client.">
