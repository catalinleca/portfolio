---
title: "HedgeHunt"
description: "How I built a platform where creators and explorers share the same data without breaking each other's experience."
date: "2024-01-01"
...
<ExecutiveSummary
  problem="Creators edit hunts while explorers are actively solving them. If a creator changes step 3 while an explorer is on step 1, the explorer could eventually reach content that no longer exists. Editing and playing need to be completely isolated."
  ...
  architecture="TypeScript monorepo where every app shares types generated from a single OpenAPI spec. MongoDB with a versioning model that isolates drafts from live content. A server-side exporter (PlayerExporter) strips answer keys before anything reaches the explorer."
...
HedgeHunt lets creators build multi-step treasure hunts in a visual builder. Explorers solve them on mobile using GPS, photos, and audio. Hunts are made of steps, and each step can be a different type of challenge. A Clue asks explorers to solve a text riddle. A Quiz tests knowledge with multiple choice or free-text. A Mission sends them to GPS coordinates to capture photos or audio. A Task accepts open-ended submissions that AI validates.

The Builder and Explorer are separate apps because they serve different users with different trust levels. Creators get a real-time preview through an iframe that runs the actual Explorer, with answer keys stripped server-side.

The server controls game progression. Each API response tells the Explorer what actions are available next. Explorers never decide what comes next.
