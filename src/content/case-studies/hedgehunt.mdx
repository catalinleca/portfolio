...
<ExecutiveSummary
  problem="Creators design interactive treasure hunts while explorers are out solving them. Four challenge types, three AI providers, and a publishing workflow where editing can never break a live hunt."
  constraints="Four apps deployed independently need to stay in sync. Published content must be immutable. AI validation must never block an explorer mid-hunt."
  architecture="TypeScript monorepo where every app shares types generated from a single OpenAPI spec. MongoDB with a versioning model that isolates drafts from live content. A server-side transformation layer ensures explorers never see answer keys."
  results="Publish and roll back without downtime. Explorers play version-locked sessions that never change mid-hunt. Live at hedgehunt.app."
/>
...
HedgeHunt lets creators build multi-step treasure hunts in a visual builder. Explorers solve them on mobile using GPS, photos, and audio. Four challenge types cover different kinds of interactions: Clue for text-based riddles, Quiz for multiple choice and free-text answers, Mission for GPS verification and media capture, and Task for AI-validated submissions.

The Builder and Explorer are separate apps. Creators get real-time preview through an iframe that runs the actual Explorer app, with answer keys and sensitive data removed. The server controls game progression: each API response tells the Explorer what actions are available next. Explorers never decide what comes next.
...
`@hunthub/shared` is the single source of truth for all types and validation schemas. They're generated from one OpenAPI spec, and every app imports from here. If the spec changes and an app doesn't update, CI catches it at build time.

`@hunthub/compass` is the shared design system. It wraps MUI's `createTheme` with custom tokens, typography, and component overrides. Any app that imports it gets a consistent look without duplicating configuration.

`@hunthub/player-sdk` handles communication between Builder and Explorer. The Builder sends hunt data to the Explorer iframe through a typed messaging layer. A `PlayerExporter` strips answer keys and AI instructions before anything reaches the Explorer.

<Mermaid chart={`graph TD
  A[HuntHub Monorepo] --> B[Builder App]
  A --> C[Explorer App]
  A --> D[Backend API]
  A --> E[Landing Page]
  A --> F["@hunthub/shared"]
...
The API runs on Fly.io. Builder and Explorer deploy to Vercel. The landing page ships to Cloudflare Pages. Static assets go through S3 with CloudFront as the CDN.
...
I went with the three-model approach. `Hunt` is the master record with a `liveVersion` field pointing to whichever `HuntVersion` is currently live. Publishing clones the draft steps into a new `HuntVersion`. Releasing swaps the pointer. Rolling back swaps it to a different version.
...
The filter `liveVersion: currentLiveVersion` acts as optimistic locking. If someone else released between the read and the write, the filter matches nothing and the server throws a conflict. No distributed locks, no queues.
...
For numeric IDs (MongoDB defaults to ObjectIds), I built a `Counter` collection. Each model has a counter document. `findOneAndUpdate` with `$inc` gives sequential, race-condition-safe, human-readable IDs for URLs and display.

I gave up JOINs. Every "join" is an explicit batch query using `$in` on arrays of IDs, which avoids N+1 problems. Cross-collection ACID isn't available, but `withTransaction` covers the critical paths: publish, release, and delete.

MongoDB's TypeScript support isn't as strong as Prisma's. But since the OpenAPI spec is the source of truth for all types, Mongoose schemas just follow along. They're secondary, not authoritative.
...
`PlayerExporter` runs server-side. It takes a full `Step` and produces a `StepPF` containing only what the explorer needs to see and interact with. Answer keys, AI prompts, validation criteria, and admin metadata are all stripped. Quiz options are randomized at this layer so the order is consistent within a session but unpredictable across sessions.

The same exporter feeds two consumers: the play API (real explorers solving hunts) and the builder preview (creators previewing through the iframe). Both receive identical explorer-safe output. The preview always matches exactly what a real explorer would experience.
...
Four challenge types (Clue, Quiz, Mission, Task) with completely different field schemas. The builder needs to let creators switch a step between types without destroying shared data like title and description. The API needs to validate based on type. The OpenAPI spec needs to generate clean schemas for all four.
...
Two distinct AI use cases. First: validating explorer submissions for Missions (photos of specific locations or objects) and Tasks (audio recordings) where exact matching is impossible. Second: generating complete, playable hunts from a text prompt like "a treasure hunt around Central Park about American history." Multiple AI providers handle different media types, and provider failures can never block an explorer mid-hunt.
...
For validation, GPT-4o handles text validation and Google Gemini handles image and audio analysis. If a provider fails or times out at 15 seconds, validation auto-passes with a `fallbackUsed` flag. Explorers are never blocked by infrastructure.
...
For generation, I use OpenAI Structured Outputs with a Zod schema derived from the API types. The schema intentionally omits GPS coordinates from Missions, since the model would hallucinate them. Instead, it generates a text description of the location.

After generation, each step goes through a geocoding pipeline. Google Maps converts the location description to coordinates, and only high-precision results are accepted. If geocoding fails or the precision is too low, the step silently converts from a Mission to a Clue. Every generated hunt is immediately playable. If schema validation fails, one retry feeds the specific Zod errors back to the model.

</CardField>
<CardField label="Tradeoff">

Auto-pass on validation failure means a bad answer could slip through. But blocking explorers when AI is down is worse for the experience. The generation quality gate trades geographic coverage for accuracy. Some locations become Clues instead of GPS challenges, but every generated hunt is immediately playable without manual fixes.
...
<CodeShowcase title="Answer Validation" description="Six validators behind one interface. Each challenge type maps to a validator: fuzzy text matching for quiz inputs, GPS distance checks for location missions, and AI analysis for image and audio. Adding a new validator means one map entry. Zero changes to dispatch logic.">
...
The Explorer prefetches next-step media with async `decode()` and a bounded LRU cache. Step transitions feel instant because assets are already decoded.
...
<CodeShowcase title="Play API and HATEOAS" description="Each API response tells the Explorer what it can do next. The Explorer follows server-provided links, it never decides on its own. Game rules live on the server, so they can change without touching the client.">
