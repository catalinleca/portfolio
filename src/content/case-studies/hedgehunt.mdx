...
HedgeHunt lets creators build multi-step treasure hunts in a visual builder and explorers solve them on mobile using GPS, photos, and audio. The platform supports four challenge types: Clue (text-based riddles), Quiz (multiple choice or free-text input), Mission (GPS location verification and media capture), and Task (AI-validated audio and image submissions).

The Builder and Explorer are separate apps. Creators get real-time preview through an iframe that runs the actual Explorer app with stripped-down hunt data. The server controls game progression through hypermedia links, where each API response tells the client what actions are available. Explorers never decide what comes next.
...
`@hunthub/player-sdk` handles the typed postMessage protocol between Builder and Explorer. The Builder sends hunt data through a `PlayerExporter` that strips answer keys and AI instructions before the iframe receives them.
...
I gave up JOINs. Every "join" is an explicit batch query using `$in` on arrays of IDs to avoid N+1 patterns. I gave up cross-collection ACID, but `withTransaction` covers the critical paths: publish, release, and delete.
...
Sessions are version-locked. Releasing a new version mid-play doesn't interrupt active explorers. `localStorage` persistence resumes from the last completed step.

</HighlightCard>
<HighlightCard title="Builder Preview">

The Builder embeds the Explorer in an iframe via a typed `postMessage` protocol. Same `PlayerExporter` as production. Preview matches exactly what real explorers see.
...
<CodeShowcase title="Play API and HATEOAS" description="Each API response includes hypermedia links for available actions. The Explorer follows links, it never decides what comes next. Game rules change server-side without touching the client.">
