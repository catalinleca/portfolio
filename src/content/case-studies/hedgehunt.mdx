...
<DecisionCard id="ai-hunt-editing" title="AI Hunt Editing" variant="breakout">
<CardSection>
<CardField label="Context">
...
- **Traditional agent loop.** One model inference per tool call. The model calls `getHunt`, waits for the result, decides what to change, calls `saveHunt`, waits again, writes a summary. A three-tool edit needs four inferences. Every tool result feeds back into the context window, so the model re-reads the full hunt JSON on each turn.
...
Claude's Programmatic Tool Calling. On the first inference, the model writes a Python script. The script calls `getHunt` to read the current state, modifies steps in memory, calls `geocodeLocations` for new location-based missions, and calls `saveHunt` with the complete steps array. Each tool call pauses the script. The backend executes it against real services and resumes with the result. When the script finishes, Claude reads the output and writes a summary on the second inference.

</CardField>
</CardSection>
<CardBreakout>
  <ProcessFlow title="Traditional Agent Loop">
    <ProcessStep label="Inference 1" detail="Decide to call getHunt" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
    <ProcessStep label="Inference 2" detail="Decide to call saveHunt" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
    <ProcessStep label="Inference 3" detail="Decide to call geocodeLocations" />
    <ProcessStep label="Execute" detail="Run tool, return result to model" />
    <ProcessStep label="Inference 4" detail="Write summary" />
  </ProcessFlow>
  <ProcessFlow title="Programmatic Tool Calling">
    <ProcessStep label="Inference 1" detail="Write Python script with all tool calls" />
    <ProcessStep label="Execute" detail="Script runs, pauses on each tool call" />
    <ProcessStep label="Inference 2" detail="Read stdout, write summary" />
  </ProcessFlow>
</CardBreakout>
<CardSection>
<CardField label="Decision">
...
I started with seven tools: `getHunt`, `getStep`, `addStep`, `updateStep`, `removeStep`, `updateHunt`, and `geocodeLocation`. Most turned out to be wasteful. `getStep` added a round-trip for no benefit since `getHunt` already returns all steps. Individual step mutations couldn't handle reordering and required multiple calls for multi-step edits. I consolidated down to four: `getHunt`, `updateHunt`, `saveHunt`, and `geocodeLocations`. `saveHunt` accepts the full steps array and saves atomically. Adding, removing, reordering, and updating steps are all one call.

</CardField>
<CardField label="Tradeoff">

Tied to Claude's code execution API. The model must produce valid Python in a single pass, so the system prompt needs precise tool documentation and examples. Container startup adds latency to the first call. But for edits that touch multiple steps and need geocoding, the inference reduction is significant. The model makes all creative decisions at code-writing time. Everything after that is execution.
...
<HighlightGrid>
<HighlightCard title="Durable Analytics">

Anonymous play sessions expire after 24 hours. If analytics read raw sessions directly, long-term metrics would decay as old documents are deleted. Every play event writes to two stores: the session record (temporary) and a `HuntStats` document with running totals (permanent). Averages are computed at read time from the totals, never stored. The analytics service reads both, so recent sessions have full detail and lifetime metrics are always accurate.

</HighlightCard>
<HighlightCard title="Fire-and-Forget Stats">

Stats writes happen after the game transaction commits, asynchronously. If the analytics write fails, the player's progression is unaffected. Counters update at two boundaries: when a player starts a session and when they finish. Nothing in between. This prevents double-counting and keeps the play path responsive.
...
<HighlightGrid>
<HighlightCard title="Form Auto-Save">

Before sending an instruction to the AI, the editor saves the current form state. The model always edits the latest data, not whatever was last persisted. After the edit completes, the form resets to the returned hunt while preserving the creator's step selection.

</HighlightCard>
<HighlightCard title="Prompt Trimming">

The system prompt includes four fields per step: ID, type, title, mission type. Full data is available through `getHunt`. The model gets enough context to decide what to change without processing 50 fields per step on every request.
