...
<HighlightGrid>
<HighlightCard title="Play API and HATEOAS">

Each API response includes HATEOAS links for available actions. The Player follows links. Game rules change server-side without touching the client.

</HighlightCard>
<HighlightCard title="Answer Validation">

Six validators behind one interface, from Levenshtein distance for fuzzy text to Haversine for GPS proximity to AI for image/audio. Adding new validators required zero changes to dispatch.

</HighlightCard>
<HighlightCard title="Asset Pipeline">

Direct-to-S3 uploads via presigned URLs, served through CloudFront. Hunt-prefixed keys enable batch deletion on hunt removal.
...
Uploaded videos and audio are compressed server-side before final storage. Originals are replaced after processing for consistent playback across devices.

</HighlightCard>
<HighlightCard title="Type Generation">

A single OpenAPI spec generates TypeScript types and Zod schemas via `openapi-zod-client`. Spec changes that break any consumer fail CI at build time.
