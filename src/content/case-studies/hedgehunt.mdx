...
<ExecutiveSummary
  ...
  approach="A versioning model isolates drafts from live content. Publishing creates an immutable snapshot. A server-side exporter (PlayerExporter) strips answer keys before anything reaches the explorer. All four apps share types generated from a single OpenAPI spec."
...
Creators start with an empty hunt and add steps through a visual editor. Each step is a different type of challenge. A Clue asks explorers to solve a text riddle. A Quiz tests knowledge with multiple choice or free-text answers. A Mission sends them to GPS coordinates to capture photos or audio. A Task accepts open-ended submissions that AI validates. The Builder includes a live preview, an iframe running the actual Explorer with answer keys stripped server-side. When the hunt is ready, the creator publishes it. Publishing freezes the content into an immutable snapshot. Releasing points live traffic to that snapshot.
